...
BlockHeader *mm_block_next(BlockHeader *bp) {
    ...
    //printf("mm_block_next: current = %p, size = %d, next_address = %p\n", bp, this_size, next_addr);

    if (next_addr >= mem_heap_hi()) {
        //printf("mm_block_next: next_addr %p is out of heap bounds\n", next_addr);
        return NULL;
    }
